...
class Dockers(Component):
    def __init__(self, window: Window):
        ...
        for dock in window.dockers():
            dock.setProperty("onDockMoved", lambda _,d=dock: self.onDockMoved(d))
    ...
    def onDockMoved(self, dock):
        if not getOpt("toggle", "toggle_docking"):
            return
        if dock.isFloating():
            dock.setAllowedAreas(Qt.AllDockWidgetAreas if self._dockingEnabled else Qt.NoDockWidgetArea)
